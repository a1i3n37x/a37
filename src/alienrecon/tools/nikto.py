...
class NiktoTool(CommandTool):
    ...
    def build_command(
        ...
        **kwargs,
        ...
        Args:
            ...
            nikto_arguments: Optional string of additional arguments provided by
                             user/LLM.
        ...
        command_args = [
            ...
            "-o",
            temp_json_output_path,  # <<< Output to temp file
            "-Tuning",
            self.DEFAULT_TUNING,  # Apply default tuning
            "-nointeractive",  # Ensure no interactive prompts
            "-ask",
            "no",  # Disable auto-yes to prompts
        ]
    ...
    def parse_output(
        ...
        **kwargs,
        ...
        Args:
            stdout: Raw standard output from Nikto execution (may contain
                    progress/errors).
        ...
        # Case 2: Execution completed, but with potential warnings/errors in stderr
        if stderr:
        ...
        try:
            # Nikto JSON usually has 'host', 'port', 'banner', 'vulnerabilities'
            ...
            if isinstance(vulnerabilities_list, list):
                for item in vulnerabilities_list:
                    if vuln_count >= limit_per_type:
                        truncated = True
                        break
                    # Extract key info - structure can vary slightly based on
                    # Nikto version/plugins
                    finding_detail = {
                ...
                logging.warning(
                    "Expected 'vulnerabilities' to be a list in Nikto JSON, "
                    f"got {type(vulnerabilities_list)}"
                )
                findings["warning"] = (
                    findings.get("warning") or ""
                ) + "; Unexpected format for 'vulnerabilities' field."

            if findings["vulnerabilities"]:
                scan_summary += (
                    f" Found {len(findings['vulnerabilities'])} potential findings."
                )
            elif not findings["error"]:  # No vulns found and no errors reported
                scan_summary += (
            ...
            findings["error"] = (
                findings.get("error") or ""
            ) + f"; JSON Processing Error: {str(e)}"
            findings["raw_json_sample"] = str(parsed_json_data)[
                :500
            ]  # Include sample on error
    ...
    def execute(self, **kwargs) -> dict[str, Any]:
        ...
        Args:
            **kwargs: Arguments expected by build_command (target, port,
                      nikto_arguments).
        ...
        try:
            ...
            FileNotFoundError,
            OSError,
        ) as e:  # Catch specific errors during setup
            ...
            if temp_json_path and os.path.exists(temp_json_path):
                try:
                    ...
                    logging.warning(
                        f"Could not remove temp file {temp_json_path} after "
                        f"setup error: {cleanup_err}"
                    )
            return {
        ...
        try:
            ...
            console.print(
                f"[yellow]Initiating {self.name} scan on {target_display} "
            ...
            if temp_json_path and os.path.exists(temp_json_path):
                if os.path.getsize(temp_json_path) > 0:
                    try:
                        ...
                        err_msg = (
                            f"Failed to decode Nikto JSON from {temp_json_path}: "
                            f"{json_err}"
                        )
                        logging.error(err_msg)
                        ...
                        err_msg = (
                            f"Failed to read Nikto JSON file {temp_json_path}: "
                            f"{read_err}"
                        )
                        logging.error(err_msg)
                        if not stderr:
                    ...
                    if not stderr:
                        stderr = warn_msg  # Promote to error
            elif not stderr:  # File not exist AND command didn't report error
                error_msg = (
                    f"Nikto completed but expected JSON file was not found: "
                    f"{temp_json_path}"
                )
            ...
            if temp_json_path and os.path.exists(temp_json_path):
                try:
                    os.remove(temp_json_path)
                    logging.debug(
                        f"Removed temporary Nikto JSON file: {temp_json_path}"
                    )
                except OSError as e:
                    logging.warning(
                        f"Could not remove temporary Nikto JSON file "
                        f"{temp_json_path}: {e}"
                    )
